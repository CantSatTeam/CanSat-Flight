...
def read_raster_for_cv(ds: rasterio.io.DatasetReader, window: Window | None = None) -> np.ndarray:
    if cv2 is None:
        raise RuntimeError("OpenCV is not available; ORB refinement cannot run on this Pi build")
...
def orb_refine(
    ...
    min_good: int = 30,
    min_inliers: int = 15,
    camera_max_dim: int = 640,
    mapchip_max_dim: int = 1024,
    nfeatures: int = 1500,
) -> ORBResult | None:
    if cv2 is None:
        return None
...
def crop_dataset_to_geotiff(
    ...
    data = ds.read(window=win, boundless=True, fill_value=fill_value)
...
def gps_crop_only(
    *,
    ...
    with rasterio.open(dsm_path) as dsm_ds:
        ...
        gps_x_dsm, gps_y_dsm = latlon_to_mapxy(dsm_ds, gps_lat, gps_lon)
        dsm_crop_meta = crop_dataset_to_geotiff(
            ds=dsm_ds,
            center_x=float(gps_x_dsm),
            center_y=float(gps_y_dsm),
...
def localize_and_crop(
    *,
    ...
    max_refinement_shift_m: float = 35.0,
    require_orb: bool = False,
    out_dsm: str = "cropped_dsm.tif",
) -> dict:
    if not use_orb:
        return gps_crop_only(
            dsm_path=dsm_path,
            gps_lat=gps_lat,
            gps_lon=gps_lon,
            final_crop_m=final_crop_m,
            out_dsm=out_dsm,
        )

    if camera_path is None:
        raise RuntimeError("camera_path is required when use_orb=True")
